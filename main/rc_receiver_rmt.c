...
IRAM_ATTR static void rmt_isr_handler(void* arg){
...
void rc_receiver_rmt_init(void)
{
    ...
    for (i = 0; i < RC_NUM_CHANNELS; i++) {
        ...
        rmt_config_t rmt_tx = RMT_DEFAULT_CONFIG_RX((gpio_num_t) RECEIVER_GPIOS[i], (rmt_channel_t) RECEIVER_CHANNELS[i]);
        memcpy(&rmt_channels[i], &rmt_tx, sizeof(rmt_tx));
